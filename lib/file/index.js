...
const outputUMLFiles = async (xlsxContent) => {
  ...
  const distinctWorkstreamList = distinct(xlsxContent['Data Dict'].map((x) => x['Workstream'])).filter(
    (x) => x && x !== '',
  );

  distinctWorkstreamList.forEach((workstream) => {
    const dataDict = xlsxContent['Data Dict'].filter((x) => x['Workstream'] === workstream);
